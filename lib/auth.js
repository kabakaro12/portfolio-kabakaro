...
function signUser(user) {
  return jwt.sign(
    { sub: String(user.id), email: user.email, firstName: user.first_name, lastName: user.last_name },
    secret(),
    { expiresIn: "7d" }
  );
}
...
function sendError(res, err) {
  ...
  res.status(code).json({ ok: false, error: code === 500 ? "Erreur serveur" : err.message });
}
