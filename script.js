...
publicQuoteForm?.addEventListener("submit",async event=>{
  ...
  try { const response=await fetch("/api/report-issue",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)}); const data=await response.json(); if(!response.ok) throw new Error(data.error||"Impossible d’envoyer la demande."); const min=Number(data.estimatedMin).toLocaleString("fr-FR"),max=Number(data.estimatedMax).toLocaleString("fr-FR"); quoteStatus.textContent=`Demande ${data.quoteId} enregistrée. Estimation : ${min} € à ${max} €. Kabakaro vous recontactera.`; quoteStatus.classList.add("success"); const sentLabel=document.createElement("span"),sentAmount=document.createElement("span"); sentLabel.className="quote-sent-label"; sentLabel.textContent="Demande envoyée"; sentAmount.className="quote-sent-amount"; sentAmount.textContent=`${min} € – ${max} €`; quoteEstimate.classList.add("confirmed"); quoteEstimate.replaceChildren(sentLabel,sentAmount); publicQuoteForm.reset(); }
