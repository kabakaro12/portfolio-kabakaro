...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "GET") {
      const keys = dayKeys();
      const values = await pipeline([["GET",VIEW_KEY], ...keys.map(key=>["GET",key]), ["LRANGE",TESTIMONIAL_KEY,0,199]]);
      const testimonials = parseItems(values[values.length-1]);
      return res.status(200).json({ views:{ total:Number(values[0]||0), today:Number(values[1]||0), week:values.slice(1,8).reduce((sum,v)=>sum+Number(v||0),0) }, testimonials, pending:testimonials.filter(x=>x.status==="pending").length });
    }
    if (req.method === "PATCH") {
