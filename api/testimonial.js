...
module.exports = async function handler(req, res) {
  ...
  if (!process.env.RESEND_API_KEY) return sendJson(res, 503, { error: "Le service d’envoi n’est pas encore configuré." });

  const ip = clean(req.headers["x-forwarded-for"]?.split(",")[0] || "unknown", 80);
  const now = Date.now();
  if (now - (recentRequests.get(ip) || 0) < 60000) return sendJson(res, 429, { error: "Veuillez attendre avant un nouvel envoi." });
  ...
  if (!name || !email || !["3", "4", "5"].includes(rating) || message.length < 20 || consent !== "yes") return sendJson(res, 400, { error: "Merci de remplir correctement tous les champs obligatoires." });
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return sendJson(res, 400, { error: "L’adresse e-mail n’est pas valide." });
  ...
  const html = `<h2>Nouveau témoignage à vérifier</h2><p><strong>Nom :</strong> ${escapeHtml(name)}</p><p><strong>E-mail :</strong> ${escapeHtml(email)}</p><p><strong>Entreprise / activité :</strong> ${escapeHtml(company || "Non renseignée")}</p><p><strong>Note :</strong> ${rating}/5</p><h3>Témoignage</h3><p style="white-space:pre-wrap">${escapeHtml(message)}</p><hr><p>L’auteur a accepté d’être contacté pour confirmer une éventuelle publication. Vérifiez son identité et son accord avant d’afficher cet avis.</p>`;
  try {
